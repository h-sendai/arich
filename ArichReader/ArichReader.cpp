...
static const char* arichreader_spec[] =
{
    "implementation_id", "ArichReader",
    "type_name",         "ArichReader",
    "description",       "ArichReader component",
    "version",           "1.0",
...
ArichReader::ArichReader(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_OutPort("arichreader_out", m_out_data),
      m_sock(0),
      m_recv_byte_size(0),
      ...
      m_debug(false)
{
    ...
    // Set OutPort buffers
    registerOutPort("arichreader_out", m_OutPort);

    init_command_port();
    init_state_table();
    set_comp_name("ARICHREADER");
}

ArichReader::~ArichReader()
{
}

RTC::ReturnCode_t ArichReader::onInitialize()
{
    if (m_debug) {
        std::cerr << "ArichReader::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t ArichReader::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int ArichReader::daq_dummy()
{
    return 0;
}

int ArichReader::daq_configure()
{
    std::cerr << "*** ArichReader::configure" << std::endl;
...
int ArichReader::parse_params(::NVList* list)
{
...
int ArichReader::daq_unconfigure()
{
    std::cerr << "*** ArichReader::unconfigure" << std::endl;

    return 0;
}

int ArichReader::daq_start()
{
    std::cerr << "*** ArichReader::start" << std::endl;
...
int ArichReader::daq_stop()
{
    std::cerr << "*** ArichReader::stop" << std::endl;
...
int ArichReader::daq_pause()
{
    std::cerr << "*** ArichReader::pause" << std::endl;

    return 0;
}

int ArichReader::daq_resume()
{
    std::cerr << "*** ArichReader::resume" << std::endl;

    return 0;
}

int ArichReader::read_data_from_detectors()
{
...
int ArichReader::set_data(unsigned int data_byte_size)
{
...
int ArichReader::daq_run()
{
    if (m_debug) {
        std::cerr << "*** ArichReader::run" << std::endl;
    }
...
extern "C"
{
    ...
    {
        RTC::Properties profile(arichreader_spec);
        manager->registerFactory(profile,
                    RTC::Create<ArichReader>,
                    RTC::Delete<ArichReader>);
    }
};
