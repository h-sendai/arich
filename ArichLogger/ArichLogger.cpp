...
static const char* mylogger_spec[] = {
    "implementation_id", "ArichLogger",
    "type_name",         "ArichLogger",
...
ArichLogger::ArichLogger(RTC::Manager* manager)
    : DAQMW::DaqComponentBase(manager),
      m_InPort("arichlogger_in", m_in_data),
      ...
      m_debug(false)
{
    ...
    registerInPort("arichlogger_in", m_InPort);

    init_command_port();
    init_state_table();
    set_comp_name("ArichLogger");
}

ArichLogger::~ArichLogger()
{
}

RTC::ReturnCode_t ArichLogger::onInitialize()
{
    if (m_debug) {
        std::cerr << "ArichLogger::onInitialize()" << std::endl;
    }

    return RTC::RTC_OK;
}

RTC::ReturnCode_t ArichLogger::onExecute(RTC::UniqueId ec_id)
{
    daq_do();
...
int ArichLogger::daq_dummy()
{
    return 0;
}

int ArichLogger::daq_configure()
{
    std::cerr << "*** ArichLogger::configure" << std::endl;
    int ret = 0;
    m_isDataLogging = false;
...
int ArichLogger::parse_params(::NVList* list)
{
    int ret = 0;
    ...
    for (int i = 0; i < length; i += 2) {
        ...
        if (sname == "isLogging") {
            ...
            if (svalue == "yes") {
                ...
                isExistParamLogging = true;
                std::cerr << "ArichLogger: Save to file: true\n";
            }
            else if (svalue == "no") {
                m_isDataLogging = false;
                isExistParamLogging = true;
                std::cerr << "ArichLogger: Save to file: false\n";
            }
        }
...
int ArichLogger::daq_unconfigure()
{
    std::cerr << "*** ArichLogger::unconfigure" << std::endl;
    if (m_isDataLogging) {
        delete fileUtils;
        if (m_debug) {
...
int ArichLogger::daq_start()
{
    std::cerr << "*** ArichLogger::start" << std::endl;
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ERROR: ArichLogger: open file failed\n";
            fatal_error_report(CANNOT_OPEN_FILE);
        }
        else {
            std::cerr << "*** ArichLogger: file open succeed\n";
            m_filesOpened = true;
        }
    }
    return 0;
...
int ArichLogger::daq_stop()
{
    std::cerr << "*** ArichLogger::stop" << std::endl;

    if (m_isDataLogging && m_filesOpened) {
        if (m_debug) {
            std::cerr << "ArichLogger::stop: close files \n";
        }
...
int ArichLogger::daq_pause()
{
    std::cerr << "*** ArichLogger::pause" << std::endl;
    return 0;
}

int ArichLogger::daq_resume()
{
    std::cerr << "*** ArichLogger::resume" << std::endl;
    return 0;
}

int ArichLogger::reset_InPort()
{
    bool ret = true;
    while (ret) {
        ret = m_InPort.read();
    ...
    if (m_debug) {
        std::cerr << "*** ArichLogger::InPort flushed\n";
    }
    return 0;
}

void ArichLogger::toLower(std::basic_string<char>& s)
{
...
int ArichLogger::daq_run()
{
    ...
    if (m_isDataLogging) {
        ...
        if (ret < 0) {
            std::cerr << "### ArichLogger: ERROR occured at data saving\n";
    ...
    if (m_debug) {
        ...
        if (seq_num % m_update_rate == 0) {
            std::cerr << "ArichLogger: loop = " << seq_num << std::endl;
            std::cerr << "\033[A\r";
        }
    }
...
extern "C"
{
    void ArichLoggerInit(RTC::Manager* manager)
    {
        RTC::Properties profile(mylogger_spec);
        manager->registerFactory(profile,
                                 RTC::Create<ArichLogger>,
                                 RTC::Delete<ArichLogger>);
    }
};
